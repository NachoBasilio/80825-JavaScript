...
gsap.to('#titulo', { x: 300, duration: 2 })
...
const agregadoraDeEventoDeBoton = () => {
	...
	arrayBoton.forEach((boton) => {
		boton.addEventListener('click', (event) => {
			...
			//Que el nombre este en la notificacion
			Swal.fire({
				title: 'Su producto esta en el carrito',
				toast: true,
				position: 'top-start',
...
terminarCompra.addEventListener('click', () => {
	...
	Swal.fire({
		title: '¿Esta seguro que quiere terminar la compra?',
		showCancelButton: true,
		confirmButtonColor: '#3085d6',
		cancelButtonColor: '#d33',
		confirmButtonText: 'Si, queremos terminar',
		...
		if (result.isConfirmed) {
			...
			Swal.fire({
				title: 'Siga comprando tranquilo',
				showConfirmButton: false,
				timer: 3000,
				timerProgressBar: true,
			})
